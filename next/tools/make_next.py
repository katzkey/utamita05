...
テスターが使う URL（…/utamita05/next/app/）は凍結し、直すのは
...
def main():
    ...
    for base, _dirs, files in os.walk(NEXT):
        for f in files:
            ...
            s2 = s2.replace("(Get-ChildItem $ex -Directory | Select-Object -First 1).FullName 'next	ools'",
                            "(Get-ChildItem $ex -Directory | Select-Object -First 1).FullName 'next\tools'")
            s2 = s2.replace('-maxdepth 3 -type d -path "*/next/tools"',
